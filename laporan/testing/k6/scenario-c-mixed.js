// AlbaSambosa — K6 Scenario C: Mixed 70/30 Read/Write (50 VU, 3 minutes)
// Target: p95 < 3s, error < 1%
// Simulates real-world traffic: 70% browsing, 30% cart + checkout.
//
// Run: k6 run docs/testing/k6/scenario-c-mixed.js
...
function browse() {
  const pages = [
    // Catalog
    () => http.get(`${BASE_URL}/menu`, { tags: { type: 'read', endpoint: 'catalog' } }),
    // Product detail
    () => http.get(`${BASE_URL}/produk/${randomProductId()}`, { tags: { type: 'read', endpoint: 'product' } }),
    // Static page
    () => http.get(`${BASE_URL}${STATIC_PAGES[Math.floor(Math.random() * STATIC_PAGES.length)]}`, { tags: { type: 'read', endpoint: 'static' } }),
  ];
...
function write() {
  ...
  if (Math.random() > 0.5) {
    ...
    check(checkoutRes, { 'write checkout: status 200': (r) => r.status === 200 });
  }
  ...
  if (Math.random() > 0.7) {
    ...
    check(trackRes, { 'write tracking: status 200': (r) => r.status === 200 });
  }
